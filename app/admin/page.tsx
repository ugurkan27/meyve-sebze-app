...
const BUCKET = "food-images"; // Supabase Storage bucket adı
...
export default function AdminPage() {
  ...
  const canSave = useMemo(() => {
    ...
    // imageFile opsiyonel ama varsa tipi png/jpg olmalı
    if (imageFile) {
      const ok =
  ...
  async function uploadToStorage(file: File) {
    ...
    // Bucket PUBLIC ise public URL al
  ...
  async function handleAdd() {
    if (!canSave) {
      alert("Lütfen bilgileri kontrol et. (Resim sadece PNG/JPEG olmalı)");
      return;
    }
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 grid gap-6 lg:grid-cols-2">
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-5">
          ...
          <select
            ...
            value={category}
            onChange={(e) =>
              setCategory(e.target.value as "Meyve" | "Sebze")
            }
          >
          ...
          {imagePreview ? (
            ...
            <p className="text-xs text-slate-500 -mt-1 mb-4">
              Resim yüklemezsen “No image” görünür.
            </p>
          )}
